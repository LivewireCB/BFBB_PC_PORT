...
RpAtomic* FindAndInstanceAtomicCallback(RpAtomic* model, void* data)
{
    ...
    for (S32 i = 0; i < numMats; i++) {
        ...
        if (material && RpMatFXMaterialGetEffects(material) != rpMATFXEFFECTNULL) {
            ...
            RpAtomicSetPipeline(model, RpMatFXGetD3D8Pipeline(rpMATFXD3D8ATOMICPIPELINE));
#else
#pragma message ("WARNING: Unknown RW platform, MatFX pipeline unsupported")
#endif
            if (RpSkinGeometryGetSkin(geom)) {
...
static U32 iModelTagUserData(xModelTag* tag, RpAtomic* model, F32 x, F32 y, F32 z, S32 closeV)
{
     ...
     if (closeV < 0 || closeV > numTags)
     {
         ...
         {
             distSqr =
                 SQR(tagList[t].v.x - x) + SQR(tagList[t].v.y - y) + SQR(tagList[t].v.z - z);
