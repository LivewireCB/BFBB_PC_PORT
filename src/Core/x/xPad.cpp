...
void xPadRumbleEnable(S32 idx, S32 enable) RIMP
{
    /*_tagxPad* p = mPad + idx;
    if (p->state != 2)
    {
...
S32 xPadUpdate(S32 idx, F32 time_passed) RIMP MIMP
{
    /*U32 new_on = 0;
...
void xPadNormalizeAnalog(_tagxPad& pad, S32 inner_zone, S32 outer_zone) RIMP
{
...
void xPadAnalogIsDigital(S32 idx, S32 enable) RIMP
{
   /* if (idx != 0)
        return;
...
// WIP.
_tagxRumble* xPadGetRumbleSlot() RIMP
{
...
void xPadDestroyRumbleChain(_tagxPad* pad) RIMP
{
    /*iPadStopRumble(pad);
...
void xPadDestroyRumbleChain(S32 idx) RIMP
{
    /*xPadDestroyRumbleChain(mPad + idx);*/
}
