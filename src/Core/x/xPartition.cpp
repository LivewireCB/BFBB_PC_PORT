...
void xPartitionReset() COMPLETE
{
}

static _tagPartLink* PartitionGetFreeLink() COMPLETE
{
    return (_tagPartLink*)xMemAllocSize(sizeof(_tagPartLink));
}

static void PartitionSpaceReset(_tagPartSpace* space) COMPLETE
{
    memset(space, 0, sizeof(_tagPartSpace));
}

static void PartitionSpaceInsert(_tagPartSpace* space, void* data) COMPLETE
{
    space->total++;
...
S32 xPartitionGetTrueIdx(_tagPartition* part, S32 x_spaces, S32 y_spaces, S32 z_spaces) COMPLETE
{
...
void xPartitionWorld(_tagPartition* part, xEnv* env, S32 x_spaces, S32 y_spaces, S32 z_spaces) COMPLETE
{
    memset(part, 0, sizeof(_tagPartition));
...
S32 xPartitionInsert(_tagPartition* part, void* insert_data, xVec3* insert_pos) COMPLETE
{
...
S32 xPartitionUpdate(_tagPartition* part, void* data, S32 old_idx, xVec3* current_pos) COMPLETE
{
    S32 cur_idx;
...
void xPartitionSpaceMove(_tagPartSpace* src, _tagPartSpace* dest, U32 data) COMPLETE
{
...
void xPartitionDump(_tagPartition*, char*) COMPLETE
{
}
