...
struct xfont {
  ...
  static xfont create(U32 id, F32 width, F32 height, F32 space,
                      iColor_tag color, const basic_rect<F32> &clip) {
    xfont r;
    r.id = id;
    r.width = width;
    r.height = height;
...
struct xtextbox {
  ...
  struct tag_type {
    substr name;
    void (*parse_tag)(jot &, const xtextbox &, const xtextbox &,
                      const split_tag &);
    void (*reset_tag)(jot &, const xtextbox &, const xtextbox &,
                      const split_tag &);
    void *context;
  };
  ...
  static const tag_entry *find_entry(const tag_entry_list &el,
                                     const substr &name);
  ...
  static xtextbox create(const xfont &font, const basic_rect<F32> &bounds,
                         U32 flags, F32 line_space, F32 tab_stop,
                         F32 left_indent, F32 right_indent) {
    xtextbox r;
    r.font = font;
    r.bounds = bounds;
    r.flags = flags;
  ...
  void render(bool cache) const { render(temp_layout(cache), 0, -1); }
  ...
  F32 yextent(const layout &l, S32 begin_jot, S32 end_jot) const {
    S32 size;
    return yextent((F32)HUGE, size, l, begin_jot, end_jot);
  }

  F32 yextent(bool cache) const { return yextent(temp_layout(cache), 0, -1); }
...
inline F32 NSCREENX(F32 scale) { return (1.0f / 640) * scale; }

inline F32 NSCREENY(F32 scale) { return (1.0f / 480) * scale; }
